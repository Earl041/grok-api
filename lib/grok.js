...
const MODEL_SLUG = 'grok-fast'; // Fastest model, kita handle internet sendiri
...
// ============================================================
// WEB SEARCH - DuckDuckGo (kita handle sendiri)
// ============================================================
async function webSearch(query, numResults = 5) {
  try {
    // Guna DuckDuckGo HTML untuk scrape results
...
// ============================================================
// WEB SCRAPE - Extract content dari URL
// ============================================================
async function webScrape(url) {
  try {
    ...
    return { url, title: url, content: `Failed to fetch: ${err.message}` };
  }
}
...
async function chatWithGrok(options) {
  const {
    ...
    enableSearch = true,   // Auto search - kita handle sendiri
  ...
  try {
    ...
    // ============================================================
    // KITA HANDLE SEARCH & SCRAPE SENDIRI (bukan Grok)
    // ============================================================
    let webResults = [];
    let scrapedContent = [];
    let enrichedMessage = message;
    
    // Extract URLs dari message untuk auto-scrape
    const urlRegex = /(https?:\/\/[^\s]+)/gi;
    const urlsInMessage = message.match(urlRegex) || [];
    const allUrls = [...new Set([...urlsInMessage, ...scrapeUrls])];
    
    // Auto-detect kalau user nak search (keywords: search, cari, google, find, etc)
    const searchKeywords = /\b(search|cari|google|find|latest|terbaru|news|berita|what is|apa itu)\b/i;
    const shouldSearch = enableSearch && (searchQuery || searchKeywords.test(message));
    
    // Web Search - kita buat sendiri
    if (shouldSearch) {
      const query = searchQuery || message.replace(searchKeywords, '').trim();
      ...
      if (webResults.length > 0) {
        ...
        enrichedMessage = `[WEB SEARCH RESULTS for "${query}"]\n${searchContext}\n\n[USER QUESTION]\n${message}`;
      }
    }
    
    // Web Scrape - kita buat sendiri
    if (enableScrape && allUrls.length > 0) {
      const scrapePromises = allUrls.slice(0, 3).map(url => webScrape(url)); // Max 3 URLs
      scrapedContent = await Promise.all(scrapePromises);
      
      const scrapeContext = scrapedContent
        .map(s => `[SCRAPED: ${s.title}]\nURL: ${s.url}\n${s.content}`)
        .join('\n\n---\n\n');
      
      enrichedMessage = `${enrichedMessage}\n\n[SCRAPED WEB CONTENT]\n${scrapeContext}`;
    }
    
    // Build API payload - guna grok-fast, disable Grok's search (kita dah handle)
    const payload = {
      temporary: false,
      ...
      imageAttachments: [],
      disableSearch: true,           // Disable Grok search - kita handle sendiri
      enableWebsiteScrape: false,    // Disable Grok scrape - kita handle sendiri
    ...
    return {
      ...
      webResults: webResults,           // Results dari kita punya search
      scrapedContent: scrapedContent,   // Content dari kita punya scrape
      generatedFiles: result.generatedFiles || [],
      extractedFiles: extractedFiles.map(f => f.name),
      model: MODEL_SLUG,
      features: {
        search: webResults.length > 0,
        scrape: scrapedContent.length > 0,
      },
    };
