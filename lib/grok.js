...
const MODEL_SLUG = 'grok-fast'; // Fastest, no internet
...
async function chatWithGrok(options) {
  const {
    ...
    enableSearch = false, // Auto search (Grok decide)
    enableScrape = false, // Auto scrape websites
  } = options;
  ...
  try {
    ...
    const payload = {
      ...
      imageAttachments: [],
      disableSearch: !enableSearch, // grok-fast = no search by default
