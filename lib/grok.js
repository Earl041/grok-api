...
// PENTING: Set env SEBELUM require chromium — package check env masa load
...
async function launchBrowser() {
  ...
  // Get executable path dulu
  const executablePath = await chromium.executablePath();
  
  // FIX 3: Set LD_LIBRARY_PATH supaya Linux loader jumpa libnss3.so dan shared libs lain
...
async function chatWithGrok(message) {
  ...
  try {
    ...
    // Check login status — lebih accurate detection
    const url = page.url();
    
    // Hanya check URL redirect, bukan page content (sebab page mungkin ada "Sign in" text somewhere)
    if (url.includes('/login') || url.includes('/signin') || url.includes('/auth')) {
      throw new Error('COOKIES_EXPIRED — Cookies dah expired atau tak valid, sila update.');
    }
    
    // Double check — cari element yang confirm user logged in
    const isLoggedIn = await page.evaluate(() => {
      // Kalau ada textarea/input untuk chat = logged in
      const hasInput = document.querySelector('textarea') || 
                       document.querySelector('[contenteditable="true"]') ||
                       document.querySelector('[role="textbox"]');
      
      // Kalau ada login button yang prominent = not logged in  
      const loginBtn = document.querySelector('button[data-testid="login-button"]') ||
                       document.querySelector('a[href*="login"]');
      
      // Ada chat input = logged in, ada login button tanpa input = not logged in
      if (hasInput) return true;
      if (loginBtn && !hasInput) return false;
      
      return true; // Default assume logged in
    });
    
    if (!isLoggedIn) {
      throw new Error('COOKIES_EXPIRED — Session tak valid, sila update cookies.');
    }
